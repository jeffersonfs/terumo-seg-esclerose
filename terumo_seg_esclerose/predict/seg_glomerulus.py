...
def predict(path: str):
    ...
    output_exp_path = Path(cfg.test_param.model_param.output_exp.glomerulus) / "masks"
    output_exp_path.mkdir(parents=False, exist_ok=True)
    id = Path(path).name.split(".")[0]
    cv2.imwrite(
        (output_exp_path / f"{id}-glomerulus.png").as_posix(),
...
def get_model_list():
    ...
    output_exp = model_param.output_exp.glomerulus
